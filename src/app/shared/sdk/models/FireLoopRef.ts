...
export class FireLoopRef<T> {
  ...
  constructor(
    ...
    this.socket.emit(
      `Subscribe.${
        !parent ? model.getModelName() : parent.model.getModelName()
      }`,
  ...
  public onRemote(method: string): Observable<any> {
    ...
    if (!this.relationship) {
      ...
      event = `${this.parent.model.getModelName()}.${
        this.relationship
      }.${event}`;
    }
  ...
  public on(
    event: string,
    filter: LoopBackFilter = { limit: 100, order: "id DESC" },
  ): Observable<T | T[]> {
    if (event === "remote") {
      throw new Error(
        'The "remote" event is not allowed using "on()" method, use "onRemote()" instead',
      );
    }
    let request: any;
    if (!this.relationship) {
  ...
  public child<T>(relationship: string): FireLoopRef<T> {
    ...
    if (!settings) {
      throw new Error(
        `Invalid model relationship ${this.model.getModelName()} <-> ${relationship}, verify your model settings.`,
      );
    }
    // Verify if the relationship model is public
    if (settings.model === "") {
      throw new Error(
        `Relationship model is private, cam't use ${relationship} unless you set your model as public.`,
      );
    }
    ...
    this.childs[relationship] = new FireLoopRef<T>(
      model,
      this.socket,
      this,
      relationship,
  ...
  private operation(event: string, data: any): Observable<T> {
    ...
    if (!this.relationship) {
      ...
      resultEvent = `${this.parent.model.getModelName()}.${
        this.relationship
      }.value.result.${this.id}`;
    }
